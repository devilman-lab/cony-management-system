...
-- 2) 入れ替え
-- ---------------------------------------------------------------------------
-- まるごと1つのトランザクションにしてあります。囲まないと、この2文の間に
-- 「一意の決まりが何も無い」すきまができ、動いているバックエンドがそこで重複した請求を
-- 作れてしまいます。そうなると CREATE が落ち、制約も索引も無い無防備な状態で残ります。
-- トランザクションにすれば DROP の時点で表を押さえるので、すきまはできません。
BEGIN;
